...
export default function AddRecipeScreen({ navigation }) {
  ...
  // Minimal arrays for UX (Using strings separated by newlines to parse later)
  ...
  const submitRecipe = async () => {
    ...
    setLoading(true);
    try {
      // 1. Convert text to arrays securely
      ...
      // Append image securely based on platform
      const filename = imageUri.split('/').pop();
      const match = /\.(\w+)$/.exec(filename);
      const type = match ? `image/${match[1]}` : `image`;

      // NOTE: On Web or specific react-native versions, we might need a blob instead. 
      // For general React Native expo uploads:
      if (Platform.OS === 'web') {
        // Fetch raw blob from URI to append for web
      ...
      showAlert('Success', 'Recipe Published successfully!', 'success');
      
      // Clear all fields to allow adding a new post
      setTitle('');
      setDescription('');
      setCookTime('');
      setServings('');
      ...
      if (categories.length > 0) {
        setCategoryIds([categories[0]._id]);
      } else {
        setCategoryIds([]);
      }
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
        <Text style={styles.header}>Publish a Recipe</Text>
        ...
        <View style={styles.row}>
          <View style={{flex:1, marginRight: 10}}>
            <RequiredLabel label="Cook Time in mins" />
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.catRow}>
          {categories.map((c) => (
            <TouchableOpacity
              key={c._id}
              style={[styles.catPill, categoryIds.includes(c._id) && styles.catPillActive]}
        ...
        <TouchableOpacity style={styles.btn} onPress={submitRecipe} disabled={loading}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>🚀 Publish Recipe</Text>}
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  btn: { backgroundColor: '#e67e22', padding: 18, borderRadius: 12, alignItems: 'center', marginTop: 30 },
